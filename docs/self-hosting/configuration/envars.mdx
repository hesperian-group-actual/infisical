...
Infisical accepts all configurations via environment variables. For a minimal self-hosted instance, at least `ENCRYPTION_KEY`, `AUTH_SECRET`, and `DB_CONNECTION_URI` must be defined. Redis is required unless `USE_IN_MEMORY_STORE=true` (single-instance / Railway serverless deployments).
...
<Tabs>
  <Tab title="Redis Standalone">
    <ParamField query="REDIS_URL" type="string" default="none">
      Redis connection string. Required unless `USE_IN_MEMORY_STORE=true`. For SSL/TLS connections, use the `rediss://` protocol (note the double 's').
...
### Low-traffic / Railway (optional Redis)

Use these when Infisical is a single replica that only serves occasional secret fetches (for example Railway with Serverless enabled). Persistent Redis connections send keepalives, which Railway treats as outbound traffic, so the service never sleeps.

<ParamField query="USE_IN_MEMORY_STORE" type="string" default="false">
  When `true`, Infisical does not connect to Redis. Cache, locks, and OAuth sessions stay in process memory. Safe for a single replica. Set `false` and provide `REDIS_URL` for multi-replica, background workers, or SSO that must survive restarts.
</ParamField>

<ParamField query="MINIMAL_SECRET_MANAGER_MODE" type="string" default="true">
  Skip recurring background sync crons (env/OAuth/license/Teams, SSE permission refresh). Secret API reads still work.
</ParamField>

<ParamField query="QUEUE_WORKERS_ENABLED" type="string" default="false">
  Start BullMQ workers. Leave `false` unless you need secret sync, rotation, audit-log jobs, etc.
</ParamField>
